...
namespace grb
{
    namespace backend
    {
...
#if 0
        ERROR AMBIGUOUS WITH PREVIOUS FUNCTION
        // *******************************************************************
        /// perform the following operation on sparse vectors implemented as
        /// vector<tuple<Index, value>>
        ///
        ...
        ///
        /// t<[m[:]]> += a_ik*b[:]
        template<typename TRowT, //TScalarT,
                 typename MRowT, //MScalarT,
                 typename SemiringT,
                 typename AScalarT,
                 typename BRowT> //BScalarT>
        void masked_axpy(
            TRowT       &t,  //std::vector<std::tuple<IndexType, TScalarT>>       &t,

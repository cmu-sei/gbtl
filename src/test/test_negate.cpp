...
BOOST_AUTO_TEST_CASE(test_negate_square)
{
    ...
    graphblas::IndexArrayType i_mA    = {0, 1, 2};
    graphblas::IndexArrayType j_mA    = {0, 1, 2};
    std::vector<double>       v_mA    = {1, 1, 1};
    Matrix<double, DirectedMatrixTag> mA(3, 3);
    buildmatrix(mA, i_mA, j_mA, v_mA);

    graphblas::IndexArrayType i_mA_n    = {0, 0, 1, 1, 2, 2 };
    graphblas::IndexArrayType j_mA_n    = {1, 2, 0, 2, 0, 1 };
    std::vector<double>   v_answer    = {1, 1, 1,1, 1, 1};
    Matrix<double, DirectedMatrixTag> answer(3, 3);
    buildmatrix(answer, i_mA_n, j_mA_n, v_answer);
...
BOOST_AUTO_TEST_CASE(test_negate_nonsquare)
{
    ...
    graphblas::IndexArrayType i_mA    = {0, 0, 1, 1, 2, 2};
    graphblas::IndexArrayType j_mA    = {0, 2, 1, 3, 2, 3};
    std::vector<double>       v_mA    = {1, 1, 1, 1, 1, 1};
    Matrix<double, DirectedMatrixTag> mA(3, 4, 0);
    buildmatrix(mA, i_mA, j_mA, v_mA);

    graphblas::IndexArrayType i_mA_n    = {0, 0, 1, 1, 2, 2};
    graphblas::IndexArrayType j_mA_n   =  {1, 3, 0, 2, 0, 1};
    std::vector<double>   v_answer    =   {1, 1, 1, 1, 1, 1};
    Matrix<double, DirectedMatrixTag> answer(3, 4, 0);
    buildmatrix(answer, i_mA_n, j_mA_n, v_answer);
    auto result = negate(mA);
...
BOOST_AUTO_TEST_CASE(test_negate_transpose_nonsquare)
{
    ...
    std::vector<double>   v_answer    =   {1, 1, 1, 1, 1, 1};
    Matrix<double, DirectedMatrixTag> answer(4,3, 0);
    buildmatrix(answer, j_mA_n, i_mA_n, v_answer);
